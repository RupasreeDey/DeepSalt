...
class DomainDiscriminator(nn.Module):
    ...
    def forward(self, x):
        x_grl = GradientReversalLayer.apply(x)  # Apply gradient reversal
        return self.fc(x_grl)
...
class FeatureDistillationLoss(nn.Module):
    ...
    def forward(self, student_features_list, teacher_features_list): # Accepts lists of features
        total_feature_loss = 0
        for student_feat, teacher_feat in zip(student_features_list, teacher_features_list):
            total_feature_loss += self.criterion(student_feat, teacher_feat) # MSE for each layer
        return total_feature_loss / len(student_features_list) # Average loss over layers
    
def multi_task_learning_loss(student_features, student_salinity_predictions, targets, loss_criterion, weight_intermediate=0.5, weight_final=0.5):
    """
    Compute task loss using multi-task learning (separate losses for intermediate features and final output).

    Args:
        student_features (torch.Tensor): Intermediate features from the student model.
        student_salinity_predictions (torch.Tensor): Final output predictions from the student model.
        targets (torch.Tensor): Ground truth targets.
        loss_criterion: Loss function (e.g., Smooth L1 loss or Huber loss).
        weight_intermediate (float): Weight for intermediate features loss (default: 0.5).
        weight_final (float): Weight for final output loss (default: 0.5).

    Returns:
        torch.Tensor: Combined task loss.
    """
    # Ensure weights sum to 1
    assert abs(weight_intermediate + weight_final - 1.0) < 1e-6, "Weights must sum to 1."

    # Compute task loss for intermediate features
    task_loss_intermediate = loss_criterion(student_features.mean(dim=1), targets.unsqueeze(1))

    # Compute task loss for final output
    task_loss_final = loss_criterion(student_salinity_predictions, targets.unsqueeze(1))

    # Combine losses
    task_loss = (weight_intermediate * task_loss_intermediate) + (weight_final * task_loss_final)
    return task_loss

def knowledge_distillation_loss(student_features, teacher_features, targets, domain_preds, student_salinity_predictions, alpha=0.3, beta=0.2, smooth_l1_loss_criterion=None, feature_distillation_loss_criterion=None, huber_loss_criterion=None):
    """Compute combined loss with feature distillation, domain adaptation, and task loss."""
    # Feature distillation loss
    feature_loss = feature_distillation_loss_criterion(student_features[-1], teacher_features[-1])
    ...
    # Task loss (regression) using multi-task learning
    task_loss = multi_task_learning_loss(
        student_features=student_features[-1],
        student_salinity_predictions=student_salinity_predictions,
        targets=targets,
        loss_criterion=huber_loss_criterion,
        weight_intermediate=0.5,
        weight_final=0.5
    )
...
def train_epoch(train_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, optimizer_enmap, optimizer_domain, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta): # Use huber_loss_criterion
    """Trains the EnMAP salinity predictor model for one epoch with domain adaptation."""
    ...
    for batch_idx, (enmap_embeddings_batch, enmap_salinity_ec_batch) in enumerate(train_loader_paired):
        ...
        domain_loss = F.binary_cross_entropy(domain_preds, torch.ones_like(domain_preds)) # Domain loss to maximize discriminator accuracy

        task_loss = huber_loss_criterion(student_features.mean(dim=1), enmap_salinity_ec_batch.unsqueeze(1)) # Use Huber Loss
        combined_loss = knowledge_distillation_loss_criterion(student_intermediate_features, teacher_intermediate_features, enmap_salinity_ec_batch, domain_preds, student_salinity_predictions, alpha, beta, None, feature_distillation_loss_criterion, huber_loss_criterion) # Pass feature_distillation_loss_criterion and huber_loss_criterion, set smooth_l1_loss_criterion to None

        # Backpropagation - Combined loss updates both student and discriminator
...
def validate_epoch(val_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta): # Use huber_loss_criterion
    """Validates the EnMAP salinity predictor model for one epoch with domain adaptation."""
    ...
    with torch.no_grad():
        for batch_idx, (enmap_embeddings_batch, enmap_salinity_ec_batch) in enumerate(val_loader_paired):
            ...
            # Compute losses
            feature_loss = feature_distillation_loss_criterion(student_intermediate_features, teacher_intermediate_features) # Use intermediate features
            domain_loss = F.binary_cross_entropy(domain_preds, torch.ones_like(domain_preds)) # Domain loss
...
def test_epoch(test_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta): # Use huber_loss_criterion
    """Evaluates the EnMAP salinity predictor model on the test set with KD and Domain Adaptation."""
    enmap_salinity_predictor.eval()
    pretrained_ftir_salinity_predictor.eval()
    domain_discriminator.eval()
...
def main():
    ...
    alpha = 0.3  # Weight for feature distillation loss
    beta = 0.2  # Weight for domain adaptation loss, incsreased to 0.4
    batch_size = 64
    lr = 0.001
    weight_decay = 1e-5
    random_state = 42
    ...
    print("Starting script with Feature-Based Distillation and Adversarial Domain Adaptation...") # Updated message
    sys.stdout.flush()
    ...
    optimizer_domain = optim.Adam(domain_discriminator.parameters(), lr=lr) # Optimizer for domain discriminator

    # Initialize loss functions
    feature_distillation_loss_criterion = FeatureDistillationLoss() # Initialize FeatureDistillationLoss
    huber_loss_criterion = nn.HuberLoss() # Initialize Huber Loss
    knowledge_distillation_loss_criterion = knowledge_distillation_loss # Use function directly

    print("Starting training and validation with Huber Loss and Adversarial Domain Adaptation...") # Updated message
    sys.stdout.flush()
    train_losses, val_losses = [], []
    for epoch in range(num_epochs):
        # Train for one epoch
        train_feature_loss, train_domain_loss, train_task_loss, train_combined_loss = train_epoch(
            train_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, optimizer_enmap, optimizer_domain, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta # Pass huber_loss_criterion
        )

        # Validate for one epoch
        val_feature_loss, val_domain_loss, val_task_loss, val_combined_loss = validate_epoch(
            val_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta # Pass huber_loss_criterion
        )
    ...
    plt.legend()
    plt.title("Training and Validation Losses with Huber Loss and ADA") # Updated title
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.tight_layout()
    plt.savefig("training_results_kd_ada_huber_loss_intermediate_features.png") # Changed filename
    plt.show()
    ...
    test_feature_loss, test_domain_loss, test_task_loss, test_combined_loss, test_mae = test_epoch(
        test_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta # Pass huber_loss_criterion
    )
    ...
    torch.save(enmap_salinity_predictor.state_dict(), "enmap_salinity_predictor_kd_ada_huber_loss_intermediate_features.pth") # Changed filename
    print("Saved EnMAP salinity predictor model with intermediate feature KD, ADA and Huber Loss.") # Changed message
    sys.stdout.flush()

...
def save_model(model, filename='ftir_salinity_model_ssae.pth'):
    torch.save(model.state_dict(), filename)
    print(f'Model saved to {filename}')
    sys.stdout.flush()

def load_model(model, filename='ftir_salinity_model_ssae.pth'):
...
def explain_integrated_gradients(model, dataset):
    dataloader = DataLoader(dataset, batch_size=len(dataset))  # Load entire dataset as a single batch
    ...
    attributions, delta = ig.attribute(inputs, target=None, return_convergence_delta=True)

    attributions_np = attributions.detach().cpu().numpy()
    delta_np = delta.detach().cpu().numpy()
    inputs_np = inputs.detach().cpu().numpy()

    avg_attributions = attributions_np.mean(axis=0).squeeze()
    plt.figure(figsize=(10, 6))
    plt.bar(range(len(avg_attributions)), avg_attributions)
    plt.xlabel("Feature Index (Embedding Dimension)")  # Updated x-label
    plt.ylabel("Average Attribution")
    plt.title("Average Feature Attributions Across Dataset")
    plt.savefig("average_attributions_embed_ftir_ssae.png")  # Updated filename
    plt.close()

    sample_index = 0
    plt.figure(figsize=(10, 6))
    plt.bar(range(attributions_np.shape[2]), attributions_np[sample_index, 0])
    plt.xlabel("Feature Index (Embedding Dimension)")  # Updated x-label
    plt.ylabel("Attribution Value")
    plt.title(f"Feature Attributions for Sample {sample_index}")
    plt.savefig(f"sample_attributions_embed_ftir_ssae_{sample_index}.png")  # Updated filename
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.hist(delta_np, bins=30, alpha=0.75, color='blue')
    plt.xlabel("Convergence Delta")
    plt.ylabel("Frequency")
    plt.title("Distribution of Convergence Deltas")
    plt.savefig("convergence_deltas_embed_ftir_ssae.png")  # Updated filename
    plt.close()
...
def train_model_with_explainability(model, train_dataset, test_dataset, num_epochs=500, batch_size=64, lr=0.001):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % 100 == 0 or epoch == num_epochs:
            print(f"Explaining model at epoch {epoch}")
            sys.stdout.flush()
            explain_integrated_gradients(model, test_dataset)  # Use test_dataset for explainability
        '''
...
if __name__ == "__main__":
    ...
    save_model(model, 'ftir_salinity_model_ssae.pth')  # Updated filename

    # Example inference
    # Load the trained model
    model = TransformerModel(input_dim=64).to(device)  # Updated input_dim to 64
    model = load_model(model, 'ftir_salinity_model_ssae.pth')  # Updated filename
    ...
    plt.tight_layout()
    plt.savefig("training_results_ftir_salinity_ssae.png")  # Updated filename
    plt.show()
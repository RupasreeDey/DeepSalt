...
def weighted_combination_task_loss(student_features, student_salinity_predictions, targets, loss_criterion, weight_intermediate=0.5, weight_final=0.5):
    """
    Compute task loss by combining intermediate features and final output in a weighted manner.
    ...
    Args:
        ...
        weight_intermediate (float): Weight for intermediate features (default: 0.5).
        weight_final (float): Weight for final output (default: 0.5).
    ...
    # Compute weighted combination of intermediate features and final output
    combined_predictions = (
        weight_intermediate * student_features.mean(dim=1) + 
        weight_final * student_salinity_predictions
    )

    # Compute task loss
    task_loss = loss_criterion(combined_predictions, targets.unsqueeze(1))
    return task_loss
...
def knowledge_distillation_loss(student_features, teacher_features, targets, domain_preds, student_salinity_predictions, alpha=0.3, beta=0.2, smooth_l1_loss_criterion=None, feature_distillation_loss_criterion=None, huber_loss_criterion=None):
    ...
    # Task loss (regression) using weighted combination of intermediate and final features
    task_loss = weighted_combination_task_loss(

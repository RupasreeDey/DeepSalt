...
# --- 2. Loss Functions ---
class AttentionBasedFeatureDistillationLoss(nn.Module):
    def __init__(self):
        super(AttentionBasedFeatureDistillationLoss, self).__init__()
    ...
    def forward(self, student_features_list, teacher_features_list):
        ...
        for student_feat, teacher_feat in zip(student_features_list, teacher_features_list):
            ...
            student_attention = self.compute_attention(student_feat)
            teacher_attention = self.compute_attention(teacher_feat)
            
            # Apply attention maps to features
            student_feat_att = student_feat * student_attention
            teacher_feat_att = teacher_feat * teacher_attention
            
            # Compute MSE loss between attended features
            total_feature_loss += self.criterion(student_feat_att, teacher_feat_att)
        return total_feature_loss / len(student_features_list)

    def compute_attention(self, x):
        # Simple attention mechanism: compute attention as the normalized sum of absolute values
        return F.softmax(torch.sum(torch.abs(x), dim=1, keepdim=True), dim=2)

def knowledge_distillation_loss(student_features, teacher_features, targets, domain_preds, student_salinity_predictions, alpha=0.3, beta=0, smooth_l1_loss_criterion=None, feature_distillation_loss_criterion=None):
    """Compute combined loss with feature distillation, domain adaptation, and task loss."""
    # Feature distillation loss
    feature_loss = feature_distillation_loss_criterion(student_features, teacher_features)  # Use the new attention-based loss
...
def main():
    ...
    feature_distillation_loss_criterion = AttentionBasedFeatureDistillationLoss()  # Use the new attention-based loss

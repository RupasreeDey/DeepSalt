...
def create_dataloaders(enmap_embeddings_np, enmap_salinity_ec_scaled_tensor, train_indices, val_indices, test_indices, batch_size=8, undersample_zeros=True):
    ...
    if undersample_zeros:
        zero_indices = (train_enmap_salinity_ec_tensor == 0).nonzero(as_tuple=True)[0]
        non_zero_indices = (train_enmap_salinity_ec_tensor != 0).nonzero(as_tuple=True)[0]

        num_zeros = len(zero_indices)

        # Take 50% of zero samples
        num_zeros_to_keep = int(num_zeros * 0.1)
        undersampled_zero_indices = zero_indices[torch.randperm(num_zeros)[:num_zeros_to_keep]]
        balanced_train_indices = torch.cat([undersampled_zero_indices, non_zero_indices])
        train_indices_for_loader = [train_indices[i] for i in balanced_train_indices.tolist()]

    else:
        train_indices_for_loader = train_indices

    train_dataset = TensorDataset(train_enmap_embeddings_tensor, train_enmap_salinity_ec_tensor)

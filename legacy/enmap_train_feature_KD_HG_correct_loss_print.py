...
def knowledge_distillation_loss(student_features, teacher_features, targets, domain_preds, student_salinity_predictions, alpha=0.3, beta=0.2, smooth_l1_loss_criterion=None, feature_distillation_loss_criterion=None, huber_loss_criterion=None):
    ...
    '''
    # Combine losses
    #combined_loss = alpha * feature_loss + beta * domain_loss + (1 - alpha - beta) * task_loss
    combined_loss = alpha * feature_loss + task_loss

    return combined_loss, feature_loss, domain_loss, task_loss
...
def train_epoch(train_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, optimizer_enmap, optimizer_domain, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta):
    ...
    for batch_idx, (enmap_embeddings_batch, enmap_salinity_ec_batch) in enumerate(train_loader_paired):
        ...
        # Compute combined loss and individual losses using knowledge_distillation_loss
        combined_loss, feature_loss, domain_loss, task_loss = knowledge_distillation_loss_criterion(
            student_intermediate_features, teacher_intermediate_features, enmap_salinity_ec_batch, domain_preds, student_salinity_predictions, alpha, beta, None, feature_distillation_loss_criterion, huber_loss_criterion
        )
...
def validate_epoch(val_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta):
...
def test_epoch(test_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss_criterion, knowledge_distillation_loss_criterion, huber_loss_criterion, device, alpha, beta):
    ...
    with torch.no_grad():
        for batch_idx, (enmap_embeddings_batch, enmap_salinity_ec_batch) in enumerate(test_loader_paired):
            ...
            # Store predictions and targets for MAE, R^2, and RMSE calculation
    ...
    return test_feature_loss_epoch, test_domain_loss_epoch, test_task_loss_epoch, test_combined_loss_epoch, test_mae, test_r2, test_rmse
...
def main():
    ...
    alpha = 97  # Weight for feature distillation loss
    beta = 0  # Weight for domain adaptation loss, incsreased to 0.4
    batch_size = 64
    lr = 0.001
    weight_decay = 1e-5
    random_state = 42
    ...
    test_feature_loss, test_domain_loss, test_task_loss, test_combined_loss, test_mae, test_r2, test_rmse = test_epoch(

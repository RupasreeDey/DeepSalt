...
class TransformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return salinity_predictions, x_mean  # Return both predictions and latent features
...
def knowledge_distillation_loss(student_features, teacher_features, targets, domain_preds, student_salinity_predictions, alpha=0.5, beta=0, smooth_l1_loss_criterion=None):
...
def train_epoch(train_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, optimizer_enmap, optimizer_domain, feature_distillation_loss, knowledge_distillation_loss, smooth_l1_loss_criterion, device, alpha, beta):
    ...
    for batch_idx, (enmap_embeddings_batch, enmap_salinity_ec_batch) in enumerate(train_loader_paired):
        ...
        student_salinity_predictions, student_features = enmap_salinity_predictor(enmap_embeddings_batch)

        # Teacher model predictions
        with torch.no_grad():
            _, teacher_features = pretrained_ftir_salinity_predictor(enmap_embeddings_batch)
        ...
        '''
        print("Shape of student_salinity_predictions:", student_salinity_predictions.shape)  # Add this line
        sys.stdout.flush()
        print("Shape of enmap_salinity_ec_batch.unsqueeze(1):", enmap_salinity_ec_batch.unsqueeze(1).shape) # Add this line
        sys.stdout.flush()
        '''

        task_loss = smooth_l1_loss_criterion(student_salinity_predictions, enmap_salinity_ec_batch.unsqueeze(1))
        combined_loss = knowledge_distillation_loss(student_features, teacher_features, enmap_salinity_ec_batch, domain_preds, student_salinity_predictions, alpha, beta, smooth_l1_loss_criterion)
...
def validate_epoch(val_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss, knowledge_distillation_loss, smooth_l1_loss_criterion, device, alpha, beta):
...
def test_epoch(test_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss, knowledge_distillation_loss, smooth_l1_loss_criterion, device, alpha, beta):
...
def main():
    ...
    pretrained_ftir_salinity_predictor.load_state_dict(torch.load('ftir_salinity_model_ssae.pth', map_location=device))
    pretrained_ftir_salinity_predictor.eval()
    ...
    feature_distillation_loss = FeatureDistillationLoss()
    ...
    for epoch in range(num_epochs):
        ...
        train_feature_loss, train_domain_loss, train_task_loss, train_combined_loss = train_epoch(
            train_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, optimizer_enmap, optimizer_domain, feature_distillation_loss, knowledge_distillation_loss, smooth_l1_loss_criterion, device, alpha, beta
        )

        # Validate for one epoch
        val_feature_loss, val_domain_loss, val_task_loss, val_combined_loss = validate_epoch(
            val_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss, knowledge_distillation_loss, smooth_l1_loss_criterion, device, alpha, beta
        )
    ...
    test_feature_loss, test_domain_loss, test_task_loss, test_combined_loss, test_mae = test_epoch(
        test_loader_paired, enmap_salinity_predictor, pretrained_ftir_salinity_predictor, domain_discriminator, feature_distillation_loss, knowledge_distillation_loss, smooth_l1_loss_criterion, device, alpha, beta
    )

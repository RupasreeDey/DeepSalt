...
def knowledge_distillation_loss(student_features, teacher_features, targets, domain_preds, student_salinity_predictions, alpha=0.3, beta=0.2, smooth_l1_loss_criterion=None, feature_distillation_loss_criterion=None, huber_loss_criterion=None):
    ...
    feature_loss = feature_distillation_loss_criterion(student_features[-1], teacher_features[-1])  # Use final layer features
    ...
    '''
    #option 1
    # Task loss (regression) using final layer features
    task_loss = huber_loss_criterion(student_features[-1].mean(dim=1).squeeze(), targets.unsqueeze(1))
    '''
    #option 2
    # Concatenate features from all layers
    all_features = torch.cat(student_features, dim=1)  # Concatenate along the feature dimension
    task_loss = huber_loss_criterion(all_features.mean(dim=1).squeeze(), targets.unsqueeze(1))
    
    '''
    #option 3
    # Average features from all layers
    all_features = torch.stack(student_features, dim=1)  # Stack along a new dimension
    all_features_mean = all_features.mean(dim=1)  # Average across layers
    task_loss = huber_loss_criterion(all_features_mean.mean(dim=1).squeeze(), targets.unsqueeze(1))
    '''

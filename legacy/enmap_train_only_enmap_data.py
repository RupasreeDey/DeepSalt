...
def train_epoch(train_loader_paired, enmap_salinity_predictor, optimizer_enmap, smooth_l1_loss_criterion, epoch, num_epochs, device):
    ...
    return train_task_loss_epoch, train_salinity_mae_epoch, train_non_zero_mae_epoch, train_non_zero_loss_epoch, all_student_preds, all_enmap_ec_batches

def validate_epoch(val_loader_paired, enmap_salinity_predictor, smooth_l1_loss_criterion, device):
    ...
    return val_task_loss_epoch, val_salinity_mae_epoch, val_non_zero_mae_epoch, val_non_zero_loss_epoch, all_student_preds_val, all_enmap_ec_batches_val

def train_and_validate(num_epochs, train_loader_paired, val_loader_paired, enmap_salinity_predictor, optimizer_enmap, smooth_l1_loss_criterion, device, scheduler):
    ...
    print("Starting Training of EnMAP Salinity Predictor with AOI Split...")
    sys.stdout.flush()
    for epoch in range(num_epochs):
        train_task_loss_epoch, train_salinity_mae_epoch, train_non_zero_mae_epoch, train_non_zero_loss_epoch, all_student_preds, all_enmap_ec_batches = train_epoch(
            train_loader_paired, enmap_salinity_predictor, optimizer_enmap, smooth_l1_loss_criterion, epoch, num_epochs, device
        )
        val_task_loss_epoch, val_salinity_mae_epoch, val_non_zero_mae_epoch, val_non_zero_loss_epoch, all_student_preds_val, all_enmap_ec_batches_val = validate_epoch(
            val_loader_paired, enmap_salinity_predictor, smooth_l1_loss_criterion, device
        )
        ...
        scheduler.step(val_task_loss_epoch)

        # Early stopping logic
        if val_task_loss_epoch < best_val_loss:
            best_val_loss = val_task_loss_epoch
            counter = 0
    ...
    return train_task_losses, val_task_losses, train_salinity_mae_losses, val_salinity_mae_losses, train_non_zero_mae_losses, val_non_zero_mae_losses, train_non_zero_loss_losses, val_non_zero_loss_losses

def initialize_models_and_optimizer(enmap_reflectance_shape, device, lr=0.001, weight_decay=1e-5):
    """
    Initializes EnMAP encoder, EnMAP salinity predictor, optimizer, loss criterion, and learning rate scheduler.
    """
    ...
    return enmap_encoder, enmap_salinity_predictor, optimizer_enmap, smooth_l1_loss_criterion, scheduler

def test_model(test_loader_paired, enmap_salinity_predictor, smooth_l1_loss_criterion, device):
    ...
    return average_test_task_loss, average_test_salinity_mae, average_test_non_zero_mae, average_test_non_zero_loss, all_student_preds_test, all_enmap_ec_batches_test
...
def main():
    ...
    print("Starting script with AOI Split...")
    sys.stdout.flush()
    ...
    sys.stdout.flush()
    enmap_encoder, enmap_salinity_predictor, optimizer_enmap, smooth_l1_loss_criterion, scheduler = initialize_models_and_optimizer(enmap_reflectance.shape, device, lr, weight_decay)

    print("Starting training and validation...")
    sys.stdout.flush()
    train_task_losses, val_task_losses, train_salinity_mae_losses, val_salinity_mae_losses, train_non_zero_mae_losses, val_non_zero_mae_losses, train_non_zero_loss_losses, val_non_zero_loss_losses = train_and_validate(
        num_epochs, train_loader_paired, val_loader_paired, enmap_salinity_predictor, optimizer_enmap, smooth_l1_loss_criterion, device, scheduler
    )

    print("Evaluating on Test Set (AOI split with undersampled zeros)...")
    sys.stdout.flush()
    average_test_task_loss, average_test_salinity_mae, average_test_non_zero_mae, average_test_non_zero_loss, all_student_preds_test, all_enmap_ec_batches_test = test_model( # Corrected function call to include non_zero metrics and predictions
        test_loader_paired, enmap_salinity_predictor, smooth_l1_loss_criterion, device
    )

    print(f"Final Test Task SmoothL1 Loss (AOI split, undersampled zeros): {average_test_task_loss:.4f}, Test Salinity MAE: {average_test_salinity_mae:.4f}, Test Non-Zero MAE: {average_test_non_zero_mae:.4f}, Test Non-Zero Loss: {average_test_non_zero_loss:.4f}") # Corrected print statement to include non_zero metrics
    sys.stdout.flush()
